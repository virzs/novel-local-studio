import { AgentDelegationCard } from "@/components/assistant-ui/agent-delegation-card";
...
}> = ({ suggestions, composerExtra }) => {
  return (
    <ThreadPrimitive.Root
      className="aui-root aui-thread-root @container flex-1 flex flex-col min-h-0 overflow-hidden bg-background"
      style={{
    ...
    >
      ...
      <div className="relative mx-auto w-full max-w-(--thread-max-width) px-4 pb-4 md:pb-6">
...
const ThreadSuggestions: FC<{ suggestions: string[] }> = ({ suggestions }) => {
  return (
    <div className="aui-thread-welcome-suggestions grid w-full @md:grid-cols-2 gap-2 pb-4">
      {suggestions.map((text, i) => (
        <ThreadSuggestionItem key={i} text={text} />
      ))}
    </div>
  );
};

const ThreadSuggestionItem: FC<{ text: string }> = ({ text }) => {
  ...
  return (
    <div className="aui-thread-welcome-suggestion-display fade-in slide-in-from-bottom-2 @md:nth-[n+3]:block nth-[n+3]:hidden animate-in fill-mode-both duration-200">
      ...
      >
        <span className="aui-thread-welcome-suggestion-text-1 font-medium">{text}</span>
      </Button>
    </div>
  );
};

const Composer: FC<{ extra?: ReactNode }> = ({ extra }) => {
  return (
    <ComposerPrimitive.Root className="aui-composer-root relative flex w-full flex-col">
      ...
      >
        ...
        />
        <ComposerAction extra={extra} />
      </div>
    </ComposerPrimitive.Root>
  );
};

const ComposerAction: FC<{ extra?: ReactNode }> = ({ extra }) => {
  return (
        ...
        <MessagePrimitive.Parts>
          {({ part }) => {
            ...
            if (part.type === "tool-call") {
              ...
              if (part.toolName.startsWith("agent-"))
                return <AgentDelegationCard {...part} />;
